...
function build (BIN, THIER_DIRECTORY, project_path) {
  ...
  execSync(`${BIN}/webpack --config ${THIER_DIRECTORY}/${project_path}/config/webpack.config.js`)
  execSync(`${BIN}/node-sass ${THIER_DIRECTORY}/${project_path}/src/public/scss -o ${THIER_DIRECTORY}/${project_path}/src/public/dist/css`)
}

function buildAndWatch (BIN, THIER_DIRECTORY, project_path) {
  build(BIN, THIER_DIRECTORY, project_path)
  var tsc = spawn(`${BIN}/tsc`, ['--project', `${THIER_DIRECTORY}/${project_path}/jsconfig.json`, '--watch'])
  ...
  var webpack = spawn(`${BIN}/webpack`, ['--config', `${THIER_DIRECTORY}/${project_path}/config/webpack.config.js`, '--watch'])
  ...
  var sass = spawn(`${BIN}/node-sass`, ['-w', `${THIER_DIRECTORY}/${project_path}/src/public/scss`, '-o', `${THIER_DIRECTORY}/${project_path}/src/public/dist/css`])
...
function runDevelop (BIN, THIER_DIRECTORY, project_path, projectName) {
  buildAndWatch(BIN, THIER_DIRECTORY, project_path)
  pm2.connect(function (err) {
    ...
    pm2.start({
      name: `${projectName}`,
      cwd: `${THIER_DIRECTORY}/${project_path}`,
...
function runProduction (BIN, THIER_DIRECTORY, project_path, projectName) {
  build(BIN, THIER_DIRECTORY, project_path)
  pm2.connect(function (err) {
    if (err) {
      console.error(err)
    ...
    pm2.start({
      ...
      var server_logs = spawn(`${BIN}/pm2`, ['logs', `${projectName}`])
      server_logs.stdout.on('data', function (data) {
        console.log(goodText('server_log`s:', data))
      })
      server_logs.stderr.on('data', function (data) {
        console.log(badText('server_logs error:', data))
      })
